...
export const useLocalSequentialWorkflow = () => {
  ...
  const startWorkflow = useCallback(async (workflowData: SequentialWorkflowData) => {
    ...
    try {
      ...
      const envelope = await localDb.envelopes.update(workflowData.envelope_id, {
        workflow_stages: workflowData.stages,
        current_stage: 1,
      ...
      if (!envelope) throw new Error("Failed to update envelope");
  ...
  const processStagePayment = useCallback(async (
    ...
    setLoading(true);
    try {
      const envelope = await localDb.envelopes.selectSingle({ id: envelopeId });
      if (!envelope) throw new Error("Envelope not found");

      const currentStages = envelope.workflow_stages as WorkflowStage[];
      ...
      const updatedEnvelope = await localDb.envelopes.update(envelopeId, envelopeUpdates);
      if (!updatedEnvelope) throw new Error("Failed to update envelope");
  ...
  const rejectCurrentStage = useCallback(async (
    ...
    try {
      ...
      const updatedEnvelope = await localDb.envelopes.update(envelopeId, {
        workflow_stages: updatedStages,
  ...
  const getWorkflowStatus = useCallback(async (envelopeId: string): Promise<SequentialWorkflowData | null> => {
    try {
      ...
      const stages = (envelope.workflow_stages as WorkflowStage[]) || [];
      ...
      return {
        ...
        total_stages: stages.length,
        workflow_status: envelope.workflow_status || 'not_started',
        stages,
